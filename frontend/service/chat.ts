...
export async function GetMembersGroup(chatId: string) {
  return getJson(`group/${chatId}/memb`);
}
export async function GetProfForGroupAdd(chatId: string) {
  return getJson(`group/${chatId}`);
}
...
export async function AddProfileGroup(payload: {
  chatId: string;
  name: string;
}) {
  return putJson(
    `group/${payload.chatId}?profileId=${payload.name}`,
    undefined,
  );
}
...
export async function AddMessage(payload: messageType) {
  return postJson(`message/${payload.chatId}`, {
    format: payload.format,
    content: payload.content,
  });
}
export async function ViewMessage(msgId: string) {
  return getJson(`message/view/${msgId}`);
}
